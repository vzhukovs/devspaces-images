...
# TODO CRW-2725 when DS 3.0 is live, set CSV_VERSION_PREV="" to pull values from RHEC below
...
if [[ -z "${CSV_VERSION_PREV}" ]]; then
    ...
    if [[ -z "${CSV_VERSION_PREV}" ]]; then
        ...
        if [[ ! $(skopeo inspect docker://registry.redhat.io/devspaces/devspaces-${MIDSTM_NAME}:${DS_VERSION_PREV} --raw 2>/dev/null) ]]; then
